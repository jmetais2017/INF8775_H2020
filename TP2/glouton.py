...
def glouton(C, notes):
    ...
    # Récupréation de la transition permettant de passer de la première à la deuxième note ayant le coût le plus faible
    bestCost = np.argmin(transitions)
    # print(bestCost)
    ...
    for currentId in range(2, exSize):
        bestFinger = np.argmin(C[notes[currentId - 1]]
                               [solution[currentId - 1]][notes[currentId]])
        bestTransition = C[notes[currentId - 1]
                           ][solution[currentId - 1]][notes[currentId]][bestFinger]
