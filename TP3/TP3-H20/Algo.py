...
class Algo:
    ...
    def branchAndBound(self, size, K, print_relation):
        ...
        while len(queue) > 0:
            ...
            if node.value - score < condition_success and node.cost + cost < bound:
                solution.append(Node(node.id, total_impacts, node.value - score, node.cost + cost))
                bound = node.cost + cost
                if print_relation:
                    self.print_relation(keys, node.id, K)
                    continue
                print(str(bound) + '\n')
                continue
    ...
    def print_relation(self, keys, solution_id, K):
        for index in range(0, len(solution_id)):
            if solution_id[index] == '1':
                relations = self.contaminedBy[keys[index]]
                for i in range(0, len(relations) - K + 1):
                    print(str(relations[i]) + " " + str(keys[index]))
